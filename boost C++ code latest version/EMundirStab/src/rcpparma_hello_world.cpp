                ...
                for (int t=0; t<T_grid; t++){
                    for(int j = i+1; j < N; j++){
                        for(int l = 0; l < K; l++){
                            ...
                            int Stab_stat=(network(i,j,(grid_ids(t)-1))*network(i,j,(grid_ids(t)-2)))+((1-network(i,j,(grid_ids(t)-1)))*(1-network(i,j,(grid_ids(t)-2))));
                            t1+=(gamma(j,l)/(2*gamma(i,k)))*((Stab_stat*(theta(t,k)+theta(t,l)))-log(1+exp_val));
                        }
                    }
                }
            ...
            for(int j = i+1; j < N; j++){
                ...
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        ...
                        grad_matsub(k,l)=gamma(i,k)*gamma(j,l)*(Stab_stat-(exp_val/(1+exp_val)));
                    }
                }
            ...
            for(int j = i+1; j < N; j++){
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        ...
                        t1(t)+=(gamma(i,k)*gamma(j,l)*(Stab_stat*(theta_u(k)+theta_u(l))-log(1+exp_val)));
                    }
                }
            }
        }

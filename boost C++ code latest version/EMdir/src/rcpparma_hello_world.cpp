...
// [[Rcpp::export]]
mat grad_EM_dir_oe(mat theta_u, mat gamma, cube network, int N, int K, vec nonzero_ids, int nonzero_ids_len){
    ...
    for (int t=0; t<nonzero_ids_len; t++){
        ...
        for(int i = 0; i < (N-1); i++){
            for(int j = i+1; j < N; j++){
                ...
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        float exp_val_1=exp(theta_u(k,0));
                        float exp_val_2=exp(theta_u(l,0));
                        float exp_val_3=exp(theta_u(k,1)+theta_u(l,1));
...
// [[Rcpp::export]]
mat grad_EM_dir_re(mat theta_u, mat gamma, cube network, int N, int K, vec nonzero_ids, int nonzero_ids_len){
    mat grad_vector(nonzero_ids_len,K);
    for (int t=0; t<nonzero_ids_len; t++){
        vec grad_vec_t(K,fill::zeros);
        for(int i = 0; i < (N-1); i++){
...
// [[Rcpp::export]]
mat hess_EM_dir_oe(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index){
    cube t1(K,K,T_data);
...
// [[Rcpp::export]]
mat hess_EM_dir_re(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index){
    cube t1(K,K,T_data);
...
// [[Rcpp::export]]
mat hess_EM_dir_oe_re(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index){
    cube t1(K,K,T_data);
    mat hess_mat_1(K,K,fill::zeros);
    mat hess_mat_2(K,K,fill::zeros);
    for(int i = 0; i < (N-1); i++){
...
// [[Rcpp::export]]
float ELBO_conv_EM_dir(mat gamma, vec pi, mat theta_u, cube network, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index){
            ...
            for(int j = i+1; j < N; j++){
                for(int k = 0; k < K; k++){
                    for(int l = 0; l < K; l++){
                        ...
                        t1(t)+=(gamma(i,k)*gamma(j,l)*((indicator_10*theta_u(k,0))+(indicator_01*theta_u(l,0))+(indicator_11*(theta_u(k,1)+theta_u(l,1)))-log(1+exp_val_1+exp_val_2+exp_val_3)));
                    }
                }
            }
        }
...
// [[Rcpp::export]]
cube gamma_update_dir_CV(mat gamma, vec pi, cube theta, cube network, int N, int K, int T_grid, vec grid_ids, int test_node_set_len){
    cube quad_lin_coeff(N,K,2);
...
// [[Rcpp::export]]
mat grad_EM_dir_oe_CV(mat theta_u, mat gamma, cube network, int N, int K, vec nonzero_ids, int nonzero_ids_len, int test_node_set_len){
    mat grad_vector(nonzero_ids_len,K);
    for (int t=0; t<nonzero_ids_len; t++){
        vec grad_vec_t(K,fill::zeros);
        for(int i = 0; i < test_node_set_len; i++){
...
// [[Rcpp::export]]
mat grad_EM_dir_re_CV(mat theta_u, mat gamma, cube network, int N, int K, vec nonzero_ids, int nonzero_ids_len, int test_node_set_len){
    mat grad_vector(nonzero_ids_len,K);
    for (int t=0; t<nonzero_ids_len; t++){
        vec grad_vec_t(K,fill::zeros);
        for(int i = 0; i < test_node_set_len; i++){
...
// [[Rcpp::export]]
mat hess_EM_dir_oe_CV(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index, int test_node_set_len){
    cube t1(K,K,T_data);
    mat hess_mat_1(K,K,fill::zeros);
    mat hess_mat_2(K,K,fill::zeros);
    mat hess_mat_3(K,K,fill::zeros);
...
// [[Rcpp::export]]
mat hess_EM_dir_re_CV(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index, int test_node_set_len){
    cube t1(K,K,T_data);
    mat hess_mat(K,K,fill::zeros);
    for(int i = 0; i < test_node_set_len; i++){
        for(int j = test_node_set_len; j < N; j++){
...
// [[Rcpp::export]]
mat hess_EM_dir_oe_re_CV(mat theta_u, mat gamma, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index, int test_node_set_len){
    cube t1(K,K,T_data);
    mat hess_mat_1(K,K,fill::zeros);
    mat hess_mat_2(K,K,fill::zeros);
    for(int i = 0; i < test_node_set_len; i++){
...
// [[Rcpp::export]]
float ELBO_conv_EM_dir_CV(mat gamma, vec pi, mat theta_u, cube network, int N, int K, int T_data, float bandwidth, vec data_ids, vec grid_ids, int grid_id_index, int test_node_set_len){
    vec t1(T_data,fill::zeros);
    for (int t=0; t<T_data; t++){
        for(int i = 0; i < test_node_set_len; i++){
            for(int j = test_node_set_len; j < N; j++){

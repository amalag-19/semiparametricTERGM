        ...
        if(i!=(N-1)){
            for(int k = 0; k < K; k++){
                ...
                for (int t=0; t<nonzero_ids_len; t++){
                    ...
                    for(int j = i+1; j < N; j++){
                        for(int l = 0; l < K; l++){
                            float exp_val=exp(theta(nonzero_ids(t)-1,k)+theta(nonzero_ids(t)-1,l));
                            ...
                            quad_vec(t)+=(gamma(j,l,nonzero_ids(t)-1)/(2*gamma(i,k,nonzero_ids(t)-1)))*((Stab_stat*(theta(nonzero_ids(t)-1,k)+theta(nonzero_ids(t)-1,l)))-log(1+exp_val));
                        }
                    }
            ...
            for(int k = 0; k < K; k++){
                ...
                for (int t=0; t<nonzero_ids_len; t++){
                    ...
                    lin_vec(t)=(log(pi(nonzero_ids(t)-1,k))-log(gamma((N-1),k,nonzero_ids(t)-1))+1)*kernel_vec(t);
                }
                quad_lin_coeff(i,k,0)=accu(quad_vec);
                quad_lin_coeff(i,k,1)=accu(lin_vec);
            }
